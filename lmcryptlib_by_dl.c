		...
		switch ((func_id)) { \
			case MCRYPT_GENERIC_DEINIT_FUNC: \
				if (!generic_deinit_func) { \
					generic_deinit_func = dlsym(mcrypt_lib_handler, mcrypt_funcs[(func_id)]); \
				} \
				break; \
			case MCRYPT_ENC_GET_KEY_SIZE_FUNC: \
				if (!get_key_size_func) { \
					get_key_size_func = dlsym(mcrypt_lib_handler, mcrypt_funcs[(func_id)]); \
				} \
				break; \
			case MCRYPT_ENC_GET_IV_SIZE_FUNC: \
				if (!get_iv_size_func) { \
					get_iv_size_func = dlsym(mcrypt_lib_handler, mcrypt_funcs[(func_id)]); \
				} \
				break; \
			case MCRYPT_ENC_GET_BLOCK_SIZE_FUNC: \
				if (!get_block_size_func) { \
					get_block_size_func = dlsym(mcrypt_lib_handler, mcrypt_funcs[(func_id)]); \
				} \
				break; \
			case MCRYPT_ENC_IS_BLOCK_MODE_FUNC: \
				if (!is_block_mode_func) { \
					is_block_mode_func = dlsym(mcrypt_lib_handler, mcrypt_funcs[(func_id)]); \
				} \
				break; \
		} \
	} while (0)
